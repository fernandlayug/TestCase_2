...
def scale_data(X_train, X_test):
    ...
    save_scaler(scaler, 'model/cat_scaler_2.pkl', 'model/cat_scaler_2.xlsx', X_train.columns)
    
    return X_train_scaled, X_test_scaled

# Step 5: Save the scaler
def save_scaler(scaler, pkl_path, excel_path, feature_names):
    # Save scaler to a .pkl file
    with open(pkl_path, 'wb') as f:
        pickle.dump(scaler, f)
    
    # Save scaler parameters to Excel with feature names
    scaler_params = pd.DataFrame({'feature': feature_names, 'mean': scaler.mean_, 'var': scaler.var_})
...
def main():
    ...
    target_column = 'Completed'  # Replace 'Completed' with the actual target column name
    ...
    # Save the best model
    save_model(best_model, 'model/catboost_model_2.pkl')
