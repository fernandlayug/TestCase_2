...
# Suppress FutureWarning about the SAMME.R algorithm
...
joblib.dump(scaler, 'model/adaboost_scaler_1.pkl')
joblib.dump(encoder, 'model/adaboost_encoder_1.pkl')
...
scaler_data.to_excel('model/adaboost_scaler_1.xlsx', index=False)
...
encoder_df.to_excel('model/adaboost_encoder_1.xlsx', index=False)

# Define the AdaBoost Classifier with SAMME algorithm
...
grid_search = GridSearchCV(estimator=ada_clf, param_grid=param_grid, cv=5, n_jobs=-1, scoring='roc_auc')
...
joblib.dump(best_ada, 'model/best_ada_model_1.pkl')

# K-fold cross-validation
kf = KFold(n_splits=5, shuffle=True, random_state=42)
cv_scores = cross_val_score(best_ada, X_train_processed, y_train, cv=kf, scoring='roc_auc')
...
# ROC AUC
y_pred_prob = best_ada.predict_proba(X_test_processed)[:, 1]
...
explainer = shap.Explainer(best_ada, X_train_processed)
